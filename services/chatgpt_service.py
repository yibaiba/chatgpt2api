...
class ChatGPTService:
    ...
    def generate_with_pool(self, prompt: str, model: str, n: int, options: ImageRequestOptions | None = None):
        created = None
        ...
        for index in range(1, n + 1):
            while True:
                ...
                try:
                    result = generate_image_result(request_token, prompt, model, normalized_options)
    ...
    def edit_with_pool(
        ...
        for index in range(1, n + 1):
            while True:
                ...
                try:
                    result = edit_image_result(request_token, prompt, normalized_images, model, normalized_options)
                    account = self.account_service.mark_image_result(request_token, success=True)
                    if created is None:
                        created = result.get("created")
                    data = result.get("data")
